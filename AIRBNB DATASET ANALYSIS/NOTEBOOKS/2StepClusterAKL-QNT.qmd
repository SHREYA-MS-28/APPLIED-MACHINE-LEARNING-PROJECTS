...
# Install and load the clustering libraries
...
# Use the project folder as root, then the subfolder with CSVs
library(dplyr)

# Since your working dir is already .../bnb2_Segmentation
base_dir <- getwd()  

# Optional sanity check
list.files(base_dir, pattern = "\\.csv$")

# (optional sanity checks)
# normalizePath(base_dir)
# file.exists(file.path(base_dir, "july24.csv"))

# Read and filter each file safely (only keep existing columns)
july24_listings  <- read.csv(file.path(base_dir, "july24.csv"))  %>% select(any_of(keep_cols))
aug24_listings   <- read.csv(file.path(base_dir, "aug24.csv"))   %>% select(any_of(keep_cols))
sept24_listings  <- read.csv(file.path(base_dir, "sept24.csv"))  %>% select(any_of(keep_cols))
oct24_listings   <- read.csv(file.path(base_dir, "oct24.csv"))   %>% select(any_of(keep_cols))
nov24_listings   <- read.csv(file.path(base_dir, "nov24.csv"))   %>% select(any_of(keep_cols))
dec24_listings   <- read.csv(file.path(base_dir, "dec24.csv"))   %>% select(any_of(keep_cols))
jan25_listings   <- read.csv(file.path(base_dir, "jan25.csv"))   %>% select(any_of(keep_cols))
feb25_listings   <- read.csv(file.path(base_dir, "feb25.csv"))   %>% select(any_of(keep_cols))
march25_listings <- read.csv(file.path(base_dir, "march25.csv")) %>% select(any_of(keep_cols))
april25_listings <- read.csv(file.path(base_dir, "april25.csv")) %>% select(any_of(keep_cols))
may25_listings   <- read.csv(file.path(base_dir, "may25.csv"))   %>% select(any_of(keep_cols))
june25_listings  <- read.csv(file.path(base_dir, "june25.csv"))  %>% select(any_of(keep_cols))
...
# point to the folder that already shows in your Files pane
base_dir <- getwd()

# pick just the 12 month files in that folder
files <- list.files(
  base_dir,
  pattern = "^(july24|aug24|sept24|oct24|nov24|dec24|jan25|feb25|march25|april25|may25|june25)\\.csv$",
  full.names = TRUE
)

# helper to read + select
read_and_select <- function(file) {
  read.csv(file, stringsAsFactors = FALSE) %>%
    select(any_of(keep_cols)) %>%
...
# (optional) one big data frame with source_file column
...
```{r}
library(stringr)   # for str_replace_all, str_length, str_count
library(lubridate) # for ymd, interval, years
...
```{r}
# For mfv() = most frequent value (mode)
if (!requireNamespace("statip", quietly = TRUE)) install.packages("statip")
library(statip)

# (optional safety: if mfv isn't in scope for any reason)
if (!exists("mfv")) {
  mfv <- function(x) {
    x <- x[!is.na(x)]
...
# For k-prototypes on mixed data
if (!requireNamespace("clustMixType", quietly = TRUE)) install.packages("clustMixType")
library(clustMixType)

# For Gower distance + silhouette (you already use these later)
